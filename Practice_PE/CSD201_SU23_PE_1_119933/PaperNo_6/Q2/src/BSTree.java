    ...
    */
    void insert(String xOwner, int xPrice, int xColor) {
        ...
        if(xOwner.charAt(0) == 'B'){
          ...
          Bird b = new Bird(xOwner, xPrice, xColor);
     ...
     public void insert(Bird key) {
        ...
        while (q != null) {
            if (q.getInfo().getPrice() == key.getPrice()) {
                return;
            }
            f = q;
            if(x.getInfo().price < q.getInfo().getPrice()){
                q = q.left;
            }else{
                q = q.right;
            }
        }
        if(x.getInfo().price < f.getInfo().getPrice()){
                f.left = x;
        }else{
                f.right = x;
        }
