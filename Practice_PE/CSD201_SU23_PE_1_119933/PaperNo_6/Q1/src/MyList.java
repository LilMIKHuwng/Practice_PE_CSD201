...
public class MyList {
 ...
 Example: with the content given in the file 'data.txt', the content of 'f1.txt' after insertion 
    ...
    void addLast(String xDistrict, int xPrice, int xArea) {
        ...
        if(xPrice>0 && xArea>0){
                    Apartment a = new Apartment(xDistrict, xPrice, xArea);
    ...
    void f2() throws Exception {
        ...
        */
        ...
        if(head == null) return;
            ...
            Node cur = head;
            while (cur!=null) {                
                if(cur.getInfo().getDistrict().equalsIgnoreCase("Q2") || cur.getInfo().getDistrict().equalsIgnoreCase("Q9")){
                    cur.getInfo().setDistrict("TD");
                    cur.getInfo().setPrice(cur.getInfo().getPrice()+1);
                }
                cur = cur.next;
            }
        }
    ...
    void f3() throws Exception {
       ...
       if(head == null) return;
            ...
            Node cur = head;
            while (cur!=null) {                
                if(max.getInfo().getArea() < cur.getInfo().getArea()){
                    max = cur;
                }
                cur = cur.next;
            }
            cur = head;
             ...
             while (cur!=null) {  
                if(cur.getInfo().getArea()==max.getInfo().getArea()){
                    clear();
                    head = tail = new Node(new Apartment(cur.getInfo().getDistrict(), cur.getInfo().getPrice(), cur.getInfo().getArea()));
                    break;
                }
                cur = cur.next;
            }
//             System.out.println(head.next.getInfo());
        ...
        for(int i=0; i<n-1; i++) {
            for(int j=i+1; j<n; j++) {
                ...
                if(pi.getInfo().getPrice() < pj.getInfo().getPrice()) {
                    String district = pi.getInfo().getDistrict();
                    pi.getInfo().setDistrict(pj.getInfo().getDistrict());
                    pj.getInfo().setDistrict(district);
                    
                    int price = pi.getInfo().getPrice();
                    pi.getInfo().setPrice(pj.getInfo().getPrice());
                    pj.getInfo().setPrice(price);
                    
                    int area = pi.getInfo().getArea();
                    pi.getInfo().setArea(pj.getInfo().getArea());
                    pj.getInfo().setArea(area);
